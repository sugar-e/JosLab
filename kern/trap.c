...
trap_init(void)
{
	...
	for(i = 0; i <= 48; i++)
  	...
  	SETGATE(idt[T_BRKPT], 0, GD_KT, idt_entries[T_BRKPT], 3);
  	SETGATE(idt[T_SYSCALL], 0, GD_KT, idt_entries[T_SYSCALL], 3);
...
trap_dispatch(struct Trapframe *tf)
{
	...
	if(tf->tf_trapno==T_SYSCALL){
		result=syscall(tf->tf_regs.reg_eax, tf->tf_regs.reg_edx, tf->tf_regs.reg_ecx, tf->tf_regs.reg_ebx, tf->tf_regs.reg_edi, tf->tf_regs.reg_esi);
		if(result<0)
